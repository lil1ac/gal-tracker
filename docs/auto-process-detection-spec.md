...
| `exe_path` | 完整路径，当 match_type 为 exe_path 或 name_and_path 时必填 |
...
- 当 `match_type` 为 `exe_path` 或 `name_and_path` 时，`exe_path` 不得为空，否则不允许保存配置
...
2. 查找对应 game_id 的 session：games.current_running = true 且 ended_at IS NULL
3. 检查时长：
...
- 应用异常退出：启动时重置 current_running 状态，处理遗留 ended_at IS NULL 会话
- 进程检测失败：日志记录，继续下次轮询
- 数据库写入失败：
  - 重试 3 次，每次间隔 100ms
  - 重试失败时将数据保留在内存中，下个周期继续尝试写入
  - 内存保留最大 100 条会话，超出时丢弃最旧的
  - 确保计时数据不因数据库临时故障而丢失
- 数据库查询失败：显示错误提示，APP 继续运行（进程检测不受影响）
